...
const CATS = [
  {t:"Top 99 Asia",e:"🏆",c:"#E8723A",bg:"#FEF0E8",h:"/top100",d:"The definitive ranking of Asia’s best dive experiences. Resorts, liveaboards and dive operators ranked by professional panel, verified votes and expert review."},
  {t:"Top 99 World",e:"🌍",c:"#1B6CA8",bg:"#E8EFF8",h:"/top100-world",d:"The global edition. Every continent, every ocean. From the Red Sea to the Caribbean, the Pacific to the Mediterranean. Coming 2026."},
  {t:"Best Stays",e:"🛏️",c:"#0097A7",bg:"#E1F5F8",h:"/stays",d:"99 dive resorts ranked on room quality, service, location and how well the stay supports serious diving."},
  {t:"Best Food",e:"🍽️",c:"#C85A20",bg:"#FEF0E8",h:"/food",d:"99 dive food experiences ranked on freshness, flavour, dietary care and atmosphere. Where divers eat the best."},
  {t:"Dive Sites",e:"🤿",c:"#1B6CA8",bg:"#E8EFF8",h:"/sites",d:"The best walls, wrecks, muck dives, seamounts and shark encounters. Ranked by visibility, marine life, access and wow factor."},
  {t:"Liveaboards",e:"🚢",c:"#5E35B1",bg:"#EDE7F6",h:"/liveaboards",d:"49 liveaboards across Indonesia, Maldives, Philippines and Thailand. Comfort, food, crew, safety and dive quality all scored."},
  {t:"Sustainable",e:"🌿",c:"#2E7D32",bg:"#E8F5E9",h:"/sustainable",d:"Operators leading on reef conservation, waste reduction, community impact and certified sustainability."},
]
...
function FlipCard({c}:{c:typeof CATS[0]}) {
  ...
  return (
    <div onClick={()=>sF(!f)} style={{perspective:800,cursor:"pointer"}}>
      <div style={{position:"relative",width:"100%",height:220,transformStyle:"preserve-3d",transition:"transform .5s",transform:f?"rotateY(180deg)":"none"}}>
        <div style={{position:"absolute",inset:0,backfaceVisibility:"hidden",background:"#fff",borderRadius:14,border:"1px solid #E8E8E8",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"2rem 1.5rem",textAlign:"center",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
          <div style={{fontSize:40,marginBottom:14}}>{c.e}</div>
          <h3 style={{fontSize:16,fontWeight:700,color:"#0A2342",marginBottom:8}}>{c.t}</h3>
          <span style={{fontSize:11,color:"#aaa"}}>Tap to learn more</span>
        </div>
        <div style={{position:"absolute",inset:0,backfaceVisibility:"hidden",transform:"rotateY(180deg)",background:c.bg,borderRadius:14,border:"1.5px solid "+c.c+"30",display:"flex",flexDirection:"column",justifyContent:"space-between",padding:"1.5rem",boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
          <div>
            <h3 style={{fontSize:15,fontWeight:700,color:c.c,marginBottom:10}}>{c.t}</h3>
            <p style={{fontSize:12.5,color:"#444",lineHeight:1.65,margin:0}}>{c.d}</p>
          </div>
    ...
    <div style={{fontFamily:"Inter,system-ui,sans-serif",minHeight:"100vh",background:"#fff"}}>
      ...
      <div style={{padding:"clamp(2rem,5vw,4rem) clamp(1rem,3vw,2rem)",maxWidth:1100,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:"2.5rem"}}>
          <div style={{fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",color:"#0097A7",marginBottom:10}}>Explore Categories</div>
